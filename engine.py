...
def route(coorigin,disto,modee,temp):
  ...
  def get_aqi(lat, lon,temp):
      ...
      dct['dom_p'] = temp['dominant_pollutant']
      return [info]
      #print("Breezo meter ", dct,lat,lon)
  ...
  def f_dist(xx,yy,f1,f2):
    #m='https://graphhopper.com/api/1/matrix?point='+str(xx)+','+str(yy)+'&point='+str(f1)+','+str(f2)+'&type=json&vehicle=foot&debug=true&out_array=weights&out_array=times&out_array=distances&key=ee3859d8-5077-49a2-83ab-d8a74cfe5701'
